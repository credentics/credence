...
import 'package:pass_doc_manager/features/collections/presentation/widgets/collection_pin_dialog.dart';
...
class CollectionDashboardPage extends StatelessWidget {
  const CollectionDashboardPage({super.key, required this.collectionId});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: _CollectionDashboardView(collectionId: collectionId),
    );
  }
}

class _CollectionDashboardView extends StatefulWidget {
  const _CollectionDashboardView({required this.collectionId});

  final String collectionId;

  @override
  State<_CollectionDashboardView> createState() =>
      _CollectionDashboardViewState();
}

class _CollectionDashboardViewState extends State<_CollectionDashboardView> {
  ...
  static const double _itemCardRadius = 16;
  ...
  static const double _itemIconShellSize = 42;
  static const double _itemIconShellRadius = 13;
  static const double _itemIconSize = 19;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CollectionDetailCubit, CollectionDetailState>(
      builder: (context, state) {
        ...
        // PIN gate: if collection is locked and not yet verified, prompt.
        if (collection.isPinLocked && !_pinVerified && !_pinCheckScheduled) {
          _pinCheckScheduled = true;
          WidgetsBinding.instance.addPostFrameCallback((_) async {
            final verified = await showCollectionPinVerifyDialog(
              context: context,
              pinHash: collection.pinHash,
            );
        ...
        if (collection.isPinLocked && !_pinVerified) {
          return Scaffold(
            backgroundColor: palette.background,
            appBar: GenericAppBar(
              title: context.l10n.collectionsTitle,
        ...
        final rootBlocks =
            visibleBlocks
                .where((item) => item.parentBlockId == null)
                .toList(growable: false)
              ..sort((a, b) => a.position.compareTo(b.position));
        final showRecentlyAddedLabel =
            rootBlocks.isNotEmpty && rootBlocks.every(_isContentBlock);

        if (_isReorderMode) {
          return _reorderModeScaffold(context, collection, rootBlocks);
        }
            ...
            child: ListView(
              ...
              children: [
                ...
                if (visibleBlocks.isNotEmpty)
                  _collectionQuickStats(context, visibleBlocks),
                if (rootBlocks.isEmpty)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(22, 12, 22, 32),
                    child: _emptyInlineCard(
                      context.l10n.collectionDashboardEmptyState,
                    ),
                  )
                else ...[
                  if (showRecentlyAddedLabel)
                    Padding(
                      padding: const EdgeInsets.fromLTRB(22, 0, 22, 8),
                      child: CollectionsSectionLabel(
                        label:
                            '${context.l10n.collectionDashboardRecentlyAdded} · ${rootBlocks.length}',
                      ),
                    ),
                  ...rootBlocks.map(
                    (item) => Padding(
  ...
  Widget _collectionDetailHeader(
    ...
    final meta =
        '${context.l10n.collectionDashboardKind.toUpperCase()} · '
        '${context.l10n.collectionDetailItemCount(itemCount).toUpperCase()}';
    final details = [
      if (subtitle.isNotEmpty) subtitle,
      context.l10n.collectionDetailItemCount(itemCount),
      if (collection.isPinLocked) 'PIN',
    ].join(' · ');
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                if (_isReorderMode)
                  _collectionHeaderDoneButton(context)
                else ...[
                  _collectionHeaderIconButton(
                    context,
                    icon: collection.isPinLocked
                        ? Icons.lock_rounded
                        : Icons.more_horiz_rounded,
                    iconSize: collection.isPinLocked ? 18 : 21,
                    onTap: collection.isPinLocked
                        ? () => _togglePinLock(context)
  ...
  Widget _collectionHeaderIconButton(
    BuildContext context, {
  ...
  Widget _collectionQuickStats(
    BuildContext context,
    List<CollectionBlockEntity> blocks,
  ) {
    final folders = blocks
        .where((block) => block.type == CollectionBlockType.folder)
        .length;
    final documents = blocks
        .where((block) => block.type == CollectionBlockType.document)
        .length;
    final checklistItems = blocks
        .expand((block) => block.checklistItems)
        .toList(growable: false);
    final doneChecklist = checklistItems.where((item) => item.isDone).length;
    final reminders = blocks
        .where((block) => block.type == CollectionBlockType.reminder)
        .length;

    final thirdLabel = checklistItems.isNotEmpty
        ? context.l10n.collectionBlockChecklist
        : context.l10n.collectionBlockReminder;
    final thirdValue = checklistItems.isNotEmpty
        ...
        children: [
          Expanded(
            child: _collectionStatCard(
              context,
              label: context.l10n.collectionBlockFolder,
          ...
          Expanded(
            child: _collectionStatCard(
              context,
              label: context.l10n.collectionBlockDocument,
  ...
  Widget _collectionStatCard(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isDark ? palette.surface : const Color(0xFFF7F0DE),
  ...
  Widget _collectionHeaderDoneButton(BuildContext context) {
    final palette = context.appPalette;
    return Material(
      color: Colors.transparent,
      child: InkWell(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Text(
                context.l10n.collectionReorderInstruction,
                style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.w600,
            ...
            Expanded(
              child: ReorderableListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Padding(
                    key: ValueKey('reorder_${block.id}'),
                    padding: const EdgeInsets.only(bottom: 8),
                    child: _reorderBlockRow(context, block, index),
                  );
                },
              ),
            ),
  ...
  Widget _reorderSheetHeader(BuildContext context) {
    final palette = context.appPalette;
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              context.l10n.collectionReorderTitle,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 2),
                  Text(
                    _blockSecondaryLine(context, block),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
                      fontSize: 12,
  ...
  Widget _collectionItemCard(
    BuildContext context,
    CollectionBlockEntity block, {
    List<CollectionBlockEntity> allBlocks = const [],
  }) {
  ...
  bool _isContentBlock(CollectionBlockEntity block) {
  ...
  Widget _folderRow(
    ...
    final childCount = allBlocks
        .where((b) => b.parentBlockId == folder.id)
        .length;
    final subtitle = context.l10n.collectionDetailItemCount(childCount);
    return InkWell(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  _blockDescriptionPreview(context, folder),
                ],
              ),
            ),
            Icon(
  ...
  Widget _sectionGroup(
    ...
    final children =
        allBlocks
            .where((b) => b.parentBlockId == section.id)
            .toList(growable: false)
          ..sort((a, b) => a.position.compareTo(b.position));
    ...
    return ReorderableListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Padding(
          ...
          padding: const EdgeInsets.only(bottom: 8),
          child: _reorderBlockRow(context, child, index),
        );
      },
    );
  }
  ...
  Widget _sectionAddBlockButton(BuildContext context, String sectionId) {
    final palette = context.appPalette;
    return InkWell(
      onTap: () => _addBlockToSection(context, sectionId),
      borderRadius: BorderRadius.circular(18),
  ...
  Future<void> _addBlockToSection(
    BuildContext context,
    String sectionId,
  ) async {
    ...
    if (!context.mounted || block == null) return;
  ...
  String _blockSecondaryLine(
    BuildContext context,
    CollectionBlockEntity block,
  ) {
    return switch (block.type) {
      CollectionBlockType.folder =>
        '${context.l10n.collectionBlockFolder} · ${_childCountLabel(context, block.id)}',
      CollectionBlockType.section =>
        '${context.l10n.collectionBlockSection} · ${_childCountLabel(context, block.id)}',
      CollectionBlockType.document =>
        '${context.l10n.collectionBlockDocument} · ${[if ((block.fileType ?? '').trim().isNotEmpty) block.fileType!.trim(), if ((block.fileSizeLabel ?? '').trim().isNotEmpty) block.fileSizeLabel!.trim()].join(' · ')}',
      CollectionBlockType.note => context.l10n.collectionBlockNote,
      CollectionBlockType.input => context.l10n.collectionBlockInput,
      CollectionBlockType.checklist =>
        '${context.l10n.collectionBlockChecklist} · ${block.checklistItems.where((item) => item.isDone).length}/${block.checklistItems.length}',
      CollectionBlockType.link =>
        '${context.l10n.collectionBlockLink} · ${_domainFromUrl(block.url)}',
      CollectionBlockType.image => context.l10n.collectionBlockImage,
      CollectionBlockType.expense => context.l10n.collectionBlockExpense,
      CollectionBlockType.timeline => context.l10n.collectionBlockTimeline,
      CollectionBlockType.location => context.l10n.collectionBlockLocation,
      CollectionBlockType.reminder => context.l10n.collectionBlockReminder,
      CollectionBlockType.progress => context.l10n.collectionBlockProgress,
    };
  }

  String _childCountLabel(BuildContext context, String parentId) {
    final collection = context.read<CollectionDetailCubit>().state.collection;
    final count =
        collection?.blocks
            .where((block) => block.parentBlockId == parentId)
            .length ??
        0;
    return context.l10n.collectionDetailItemCount(count);
  }
      ...
      child: CollectionsSurfaceCard(
        ...
        child: Column(
          ...
          children: [
            _blockCardHeader(
              context,
              'EXPENSE · ${(block.metadata['category'] ?? 'General').toUpperCase()}',
              trailing: dateLabel,
              block: block,
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  _blockActionRow(
                    ...
                    label: context.l10n.collectionBlockLockWithPin,
                    showChevron: true,
                    onTap: () {
                      Navigator.of(context).pop();
                      _togglePinLock(context);
                  ...
                  _blockActionRow(
                    ...
                    label: context.l10n.collectionBlockMove,
                    showChevron: true,
                    enabled: false,
  ...
  bool _currentCollectionIsPinLocked() {
    return context
  ...
  Future<void> _togglePinLock(BuildContext context) async {
    final collection = context.read<CollectionDetailCubit>().state.collection;
    ...
    if (collection.isPinLocked) {
      // Disable PIN lock.
      final updated = collection.copyWith(isPinLocked: false, pinHash: '');
      await context.read<CollectionDetailCubit>().saveCollection(updated);
      if (!context.mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.collectionPinDisabled)));
    } else {
      // Enable PIN lock: prompt for new PIN.
      final hash = await showCollectionPinSetupDialog(context: context);
      if (hash == null || !context.mounted) return;
      final updated = collection.copyWith(isPinLocked: true, pinHash: hash);
      await context.read<CollectionDetailCubit>().saveCollection(updated);
      if (!context.mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.collectionPinEnabled)));
    }
  }
          ...
          child: Column(
            ...
            children: [
              ListTile(
                ...
                title: Text(context.l10n.editCollection),
                onTap: () {
                  Navigator.of(context).pop();
              ...
              ListTile(
                ...
                title: Text(context.l10n.collectionDashboardRearrange),
                onTap: () {
                  Navigator.of(context).pop();
                  setState(() {
                    _reorderingSectionId = null;
              ...
              ListTile(
                leading: Icon(
                  _currentCollectionIsPinLocked()
                      ? Icons.lock_open_rounded
                      : Icons.lock_rounded,
                ),
                title: Text(
                  _currentCollectionIsPinLocked()
                      ? context.l10n.collectionPinDisabled
                      : context.l10n.collectionPinEnabled,
                ),
                onTap: () {
                  Navigator.of(context).pop();
                  _togglePinLock(context);
              ...
              ListTile(
                ...
                onTap: () {
                  Navigator.of(context).pop();
                  _confirmDeleteCollection(context);
                },
              ),
            ],
          ),
  ...
  Future<void> _editCollection(BuildContext context) async {
    ...
    final draft = await Navigator.of(context).push<CollectionCreateDraft>(
      MaterialPageRoute(
        builder: (_) => CollectionsCreateStep1Page(
          isEditing: true,
          initialDraft: CollectionCreateDraft(
    ...
    final updated = collection.copyWith(
      ...
      clearIconEmoji: draft.iconEmoji == null,
      iconImagePath: draft.iconImagePath,
      clearIconImagePath: draft.iconImagePath == null,
  ...
  Future<void> _confirmDeleteCollection(BuildContext context) async {
    ...
    await context.read<CollectionDetailCubit>().deleteCollection();
    if (!context.mounted) return;
    Navigator.of(context).maybePop();
  }
